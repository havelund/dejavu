...
package sandbox.boundedquantifiers
...
class Variable(F: Formula)(name: String, bounded: Boolean, offset: Int, nrOfBits: Int) { // TODO+;
  ...
  var seen: BDD = G.False  // TODO+;
  var inRelation: BDD = G.False // TODO+;
  ...
  def getBddOf(v: Any): BDD = {
    if (bdds.contains(v)) {
      ...
      if (timeToGarbageCollect & !bounded) collectGarbage() // TODO;
...
class BDDGenerator(F: Formula)(variables: List[(String, Boolean, Int)]) { // TODO;
  var B: BDDFactory = BDDFactory.init(10000, 10000)
  val True: BDD = B.one()
  val False: BDD = B.zero()
  var offset: Int = 0
  val totalNumberOfBits: Int = variables.map(_._3).sum // TODO+;
  ...
  def initializeVariables(): Unit = {
    for ((x, b, v) <- variables) { // TODO;
      varMap += (x -> new Variable(F)(x, b, offset, v)) // TODO;
      offset += v
    }
  }
...
abstract class Formula(val monitor: Monitor) {
  ...
  /**
    * Builds a BDD from a relational expression of the form: <code>name1 op name2</code>, only
    * comparing the new values seen in this event.
    *
    * @param name1 the name of the left-hand side variable.
    * @param op    the operator.
    * @param name2 the name of the right-hand side variable.
    * @return the BDD resulting from comparing new values against previous values.
    */

  def relation(name1: String, op: RelOp, name2: String): BDD = {
    val variable1 = bddGenerator.varMap(name1)
    val variable2 = bddGenerator.varMap(name2)
    var result: BDD = bddGenerator.False
    for ((name, value, bdd) <- touchedByLastEvent) {
      if (name == name1) {
        for ((value2, bdd2) <- variable2.bdds) {
          if (op.compare(value, value2)) {
            result = result.or(bdd.and(bdd2))
            variable1.inRelation(bdd) // TODO;
            variable2.inRelation(bdd2) // TODO;
            debug(s"adding [$name1:$value] $op $name2:$value2 to '$name1 $op $name2'  BDD")
          }
        }
      }
      if (name == name2) {
        for ((value1, bdd1) <- variable1.bdds) {
          if (op.compare(value1, value)) {
            ...
            variable1.inRelation(bdd1) // TODO;
            variable2.inRelation(bdd) // TODO;
            debug(s"adding $name1:$value1 $op [$name2:$value] to '$name1 $op $name2'  BDD")
          }
        }
      }
    }
  ...
  def declareVariables(variables: (String, Boolean)*): List[Variable] = { // TODO;
    val variableList = variables.toList
    val nameList: List[String] = variableList.map(_._1) // TODO;
    val varsAndBitsPerVar = variableList.map {
      case (v,b) => (v, b, Options.BITS) // TODO;
    }
    bddGenerator = new BDDGenerator(this)(varsAndBitsPerVar)
    bddGenerator.initializeVariables()
    nameList.map(bddGenerator.varMap(_)) // TODO;
  }
...
/*
  property p: forall x . exists y . x < y & P a(x,y)
*/

class Formula_p(monitor: Monitor) extends Formula(monitor) {

  val var_x :: var_y :: Nil = declareVariables(("x", true), ("y", true)) // TODO;

  override def evaluate(): Boolean = {
    now(5) = build("a")(V("x"), V("y"))
    now(3) = relation("x", LTOP, "y").or(pre(3))
    now(4) = now(5).or(pre(4))
    now(2) = now(3).and(now(4))
    now(1) = var_y.seen.and(now(2)).exist(var_y.quantvar) // TODO;
    now(0) = var_x.seen.imp(now(1)).forAll(var_x.quantvar) // TODO;
  ...
  txt = Array(
    "forall-s x . exists y . x < y & P a(x,y)", // TODO;
    "exists-s y . x < y & P a(x,y)", // TODO;
    "x < y & P a(x,y)",
    "x < y",
    "P a(x,y)",
    "a(x,y)"
...
object TraceMonitor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = dejavu + "/src/test/scala/sandbox/boundedquantifiers/log1.csv"
    try {
