...
/*
  prop p : Forall x . closed(x) -> !telem(x)
  where
    closed(x) := !@ true | (@ closed(x) & !toggle(x)) | (@ open(x) & toggle(x))
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    ...
    now(14) = build("toggle")(V("x"))
    now(18) = build("toggle")(V("x"))
    now(24) = build("toggle")(V("x"))
    now(28) = build("toggle")(V("x"))
    // assignments2 (rule nodes excluding what is below @ and excluding leaf nodes):
    now(8) = pre(9)
    now(7) = now(8).not()
    now(11) = pre(12)
    now(13) = now(14).not()
    now(10) = now(11).and(now(13))
    now(6) = now(7).or(now(10))
    now(16) = pre(17)
    now(15) = now(16).and(now(18))
    now(5) = now(6).or(now(15))
    now(21) = pre(22)
    now(23) = now(24).not()
    now(20) = now(21).and(now(23))
    now(26) = pre(27)
    now(25) = now(26).and(now(28))
    now(19) = now(20).or(now(25))
  ...
  varsInRelations = Set()
  val indices: List[Int] = List(26,21,16,11,8)
  ...
  txt = Array(
    ...
    "!telem(x)",
    "telem(x)",
    "!@ true | (@ closed(x) & !toggle(x)) | (@ open(x) & toggle(x))",
    "!@ true | (@ closed(x) & !toggle(x))",
    "!@ true",
    "@ true",
    "true",
    "@ closed(x) & !toggle(x)",
    ...
    "open(x)",
    "toggle(x)",
    "(@ open(x) & !toggle(x)) | (@ closed(x) & toggle(x))",
    "@ open(x) & !toggle(x)",
    "@ open(x)",
    "open(x)",
    "!toggle(x)",
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("closed","telem")
