...
package sandbox.generated_monitors.rules1

// ===================================
// Simple property with renaming.
//
// property p: Forall x . r(x) -> !q(x)
//   where r(y) = p(y) | @r(y)
// ====================================
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  // TODO: DECLARE ALSO VARIABLES IN RULES
  val var_x :: var_y :: Nil = declareVariables(("x",false), ("y",false))

  // TODO: DECLARE PAIRS TO BE RENAMED
  val pair = bddGenerator.B.makePair()
  pair.set(var_y.bits,var_x.bits)

  println()
  println("--- bit patterns --- ")
  println(s"var_x.bits = ${var_x.bits.mkString("[",",","]")}")
  println(s"var_y.bits = ${var_y.bits.mkString("[",",","]")}")
  println(s"pair = $pair");
  println("---------------------")

  override def evaluate(): Boolean = {
    // TODO: UPDATE NOW AND PRE FOR RULES
    // --- rule: [r(y) = p(y) | @r(y)] ---
    now(7) = pre(8) // @r(y)
    now(6) = build("p")(V("y")); // p(y)
    now(5) = now(6).or(now(7)); // p(y) | @r(y)
    // TODO: ORDER CHANGED AND ASSIGNED TO TOP OF RULE TREE
    now(8) = now(5); // r(y) called recursively
    // --- main: [Forall x . r(x) -> !q(x)] ---
    now(4) = build("q")(V("x")) // q(x)
    now(3) = now(4).not() // !q(x)
    // TODO: RENAME
    now(2) = now(5).replace(pair); // r(x)
    now(1) = now(2).not().or(now(3)) // r(x) -> !q(x)
    now(0) = now(1).forAll(var_x.quantvar) // Forall x . r(x) -> !q(x)
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("r","q")
...
object TraceMonitor {
  def main(args: Array[String]): Unit = {
    println("Rules Example 1")
    println("An adaption of Rules requiring renaming")
