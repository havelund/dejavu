...
package sandbox.generated_monitors.rules2

// ===================================
// Simple property with NO renaming.
//
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    // TODO: UPDATE NOW FOR RULES
    // --- rule: [r(x) = p(x) | @r(x)] ---
    now(7) = pre(8) // @r(x)
    now(6) = build("p")(V("x")); // p(x)
    now(5) = now(6).or(now(7)); // p(x) | @r(x)
    // TODO: CALL RELATIONS
    now(8) = now(5); // r(x) called recursively
    now(2) = now(5) // r(x)
