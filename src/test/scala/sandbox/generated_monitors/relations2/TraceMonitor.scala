package sandbox.generated_monitors.relations2
...
class Variable(F: Formula)(name: String, offset: Int, nrOfBits: Int) {
  ...
  def getBddOf(v: Any): BDD = {
    if (bdds.contains(v)) {
      ...
      bdds += (v -> result)
      F.addTouchedByLastEvent(name, v, result)  // NEW
...
abstract class Formula(val monitor: Monitor) {
  ...
  var touchedByLastEvent: Set[(String, Any, BDD)] = null // NEW
  ...
  def relation(name1: String, op: OP, name2: String): BDD = {
    ...
    for ((name, value, bdd) <- touchedByLastEvent) {
      if (name == name1) {
        ...
        for ((value2, bdd2) <- variable2.bdds) {
          if (op.compare(value, value2)) result = result.or(bdd.and(bdd2))
        }
      }
      if (name == name2) {
        val variable1 = bddGenerator.varMap(name1)
        for ((value1, bdd1) <- variable1.bdds) {
          if (op.compare(value1, value)) result = result.or(bdd.and(bdd1))
        }
      }
  ...
  def addTouchedByLastEvent(name: String, value: Any, bdd: BDD): Unit = {
    if (touchedByLastEvent != null) touchedByLastEvent += ((name, value, bdd))
  }
...
/*
  property p: exists x . exists y . P a(x) & P b(y) & x < y
*/
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  = {
    now(5) = build("a")(V("x"))
    now(7) = build("b")(V("y")) // CHANGED ORDER: DONE
    now(4) = now(5).or(pre(4))
    now(6) = now(7).or(pre(6))
    now(3) = now(4).and(now(6))
    // now(9) = build("LT")(V("x"), V("y")) // CHANGED: removed DONE
    now(8) = relation("x", LT, "y").or(pre(8)) // CHANGED: DONE
  ...
  touchedByLastEvent = Set() // NEW
  val indices: List[Int] = List(6, 4) // CHANGED: deleted 8 - no need to GC there: DONE

  pre = Array.fill(9)(bddGenerator.False) // CHANGED: 10 -> 9 DONE
  now = Array.fill(9)(bddGenerator.False) // CHANGED: 10 -> 9 DONE

  txt = Array(
    "[0] exists x . exists y . P a(x) & P b(y) & x < y", // CHANGED : DONE
    "[1] exists y . P a(x) & P b(y) & x < y", // CHANGED : DONE
    "[2] P a(x) & P b(y) & x < y", // CHANGED : DONE
    "[3] P a(x) & P b(y)",
    "[4] P a(x)",
    "[5] a(x)",
    "[6] P b(y)",
    "[7] b(y)",
    "[8] x < y" // CHANGED: to a relation : DONE
    // "[9] LT(x,y)"     // CHANGED: removed this : DONE
  )
...
object TraceMonitor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = dejavu + "/src/test/scala/sandbox/relations/relations2/log1.csv"
    try {
