package sandbox.generated_monitors.relations3
...
abstract class Formula(val monitor: Monitor) {
  ...
  var touchedByLastEvent: Set[(String, Any, BDD)] = Set() // NEW DONE

  var varsInRelations: Set[String] = Set() // NEW DONE
...
/*
  property p: exists x . exists y . x < y & P a(x) & P b(y)
*/
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    now(6) = build("a")(V("x"))
    now(8) = build("b")(V("y"))
    now(4) = relation("x",LTOP,"y").or(pre(4))
  ...
  varsInRelations = Set("x","y") // NEW DONE
  val indices: List[Int] = List(7,5)

  pre = Array.fill(9)(bddGenerator.False)
  now = Array.fill(9)(bddGenerator.False)

  txt = Array(
    "exists x . exists y . x < y & P a(x) & P b(y)",
    "exists y . x < y & P a(x) & P b(y)",
    "x < y & P a(x) & P b(y)",
    "x < y & P a(x)",
    "x < y",
    "P a(x)",
...
object TraceMonitor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = dejavu + "/src/test/scala/sandbox/relations/relations3/log1.csv"
    try {
