...
abstract class Formula(val monitor: Monitor) {
  ...
  def relationToConstant(nameLeft: String, op: RelOp, constRight: Any): BDD = {
    val variable = bddGenerator.varMap(nameLeft)
    var result: BDD = bddGenerator.False
    for ((`nameLeft`, value, bdd) <- touchedByLastEvent if op.compare(value, constRight)) {
      result = result.or(bdd)
      variable.inRelation(bdd)
      debug(s"adding [$nameLeft:$value] $op $constRight to '$nameLeft $op $constRight'  BDD")
    }
    result
  }
...
class Formula_p(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    now(2) = build("a")(V("x"))
    now(3) = relationToConstant("x", EQOP, "5").or(pre(3)) // TODO
...
object TraceMonitor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = dejavu + "/src/test/scala/sandbox/constants/log1.csv"
