...
/*
  prop commands : Forall m . suc(m) -> Exists p . ExistsTime . !fail(m) S[<=50] dis(m,p) 
*/
...
(now(8).and(zeroTime)).or(
  ...
  .replace(u_to_t_map)
      ...
      now(3) = now(4).exist(var_p.quantvar)
  ...
  val var_m :: var_p :: Nil = declareVariables(("m",false), ("p",false))(7)
...
// End of declarations related to timed properties
  ...
  txt = Array(
    "Forall m . suc(m) -> Exists p . ExistsTime . !fail(m) S[<=50] dis(m,p)",
      "suc(m) -> Exists p . ExistsTime . !fail(m) S[<=50] dis(m,p)",
      "suc(m)",
      "Exists p . ExistsTime . !fail(m) S[<=50] dis(m,p)",
      "ExistsTime . !fail(m) S[<=50] dis(m,p)",
      "!fail(m) S[<=50] dis(m,p)",
      "!fail(m)",
      "fail(m)",
      "dis(m,p)"
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("suc","fail","dis")
