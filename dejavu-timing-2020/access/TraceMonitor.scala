...
/*
  prop access : Forall u . Forall f . access(u,f) -> ((ExistsTime . !logout(u) S[<=50] login(u)) & (ExistsTime . !close(f) S[<=50] open(f))) 
*/

class Formula_access(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(3) = build("access")(V("u"),V("f"))
      now(8) = build("logout")(V("u"))
      now(9) = build("login")(V("u"))
      now(13) = build("close")(V("f"))
      now(14) = build("open")(V("f"))
...
(now(14).and(zeroTime)).or(
  ...
  .replace(u_to_t_map)
      ...
      now(4) = now(5).and(now(10))
      now(2) = now(3).not().or(now(4))
      now(1) = now(2).forAll(var_f.quantvar)
      now(0) = now(1).forAll(var_u.quantvar)
  ...
  val var_u :: var_f :: Nil = declareVariables(("u",false), ("f",false))(7)

// Declarations related to timed properties:

  val startTimeVar : Int = 2 * Options.BITS
  val offsetTimeVar : Int = 7
  ...
  val limitMap : Map[Int,BDD] =
    Map(
          50 -> bddGenerator.B.buildCube(50,lPosArrayHighToLow)
    )

  val maxTimeLimit = 50 + 1
...
// End of declarations related to timed properties
  ...
  pre = Array.fill(15)(bddGenerator.False)
  now = Array.fill(15)(bddGenerator.False)

  txt = Array(
    "Forall u . Forall f . access(u,f) -> ((ExistsTime . !logout(u) S[<=50] login(u)) & (ExistsTime . !close(f) S[<=50] open(f)))",
      "Forall f . access(u,f) -> ((ExistsTime . !logout(u) S[<=50] login(u)) & (ExistsTime . !close(f) S[<=50] open(f)))",
      "access(u,f) -> ((ExistsTime . !logout(u) S[<=50] login(u)) & (ExistsTime . !close(f) S[<=50] open(f)))",
      "access(u,f)",
      "(ExistsTime . !logout(u) S[<=50] login(u)) & (ExistsTime . !close(f) S[<=50] open(f))",
      "ExistsTime . !logout(u) S[<=50] login(u)",
      "!logout(u) S[<=50] login(u)",
      "!logout(u)",
      "logout(u)",
      "login(u)",
      "ExistsTime . !close(f) S[<=50] open(f)",
      "!close(f) S[<=50] open(f)",
      "!close(f)",
      "close(f)",
      "open(f)"
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("login","logout","open","access","close")

  formulae ++= List(new Formula_access(this))
}
