...
/*
  prop commands : Forall m . suc(m) -> Exists p . ExistsTime . !fail(m) S[<=3] dis(m,p) 
*/
...
(now(8).and(zeroTime)).or(
  ...
  val var_m :: var_p :: Nil = declareVariables(("m",false), ("p",false))(3)

// Declarations related to timed properties:

  val startTimeVar : Int = 2 * Options.BITS
  val offsetTimeVar : Int = 3
...
// End of declarations related to timed properties
  ...
  txt = Array(
    "Forall m . suc(m) -> Exists p . ExistsTime . !fail(m) S[<=3] dis(m,p)",
      "suc(m) -> Exists p . ExistsTime . !fail(m) S[<=3] dis(m,p)",
      "suc(m)",
      "Exists p . ExistsTime . !fail(m) S[<=3] dis(m,p)",
      "ExistsTime . !fail(m) S[<=3] dis(m,p)",
      "!fail(m) S[<=3] dis(m,p)",
      "!fail(m)",
      "fail(m)",
      "dis(m,p)"
