...
/*
  prop access : Forall user . Forall file . access(user,file) -> [login(user),logout(user)) & [open(file),close(file)) 
*/
...
class Formula_access(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    ...
    // assignments5 (main formula excluding leaf nodes):
      ...
      now(1) = now(2).forAll(var_file.quantvar)
      now(0) = now(1).forAll(var_user.quantvar)
  ...
  val var_user :: var_file :: Nil = declareVariables(("user",false), ("file",false))(0)

  varsInRelations = Set()
  val indices: List[Int] = List(8,5)

  pre = Array.fill(11)(bddGenerator.False)
  now = Array.fill(11)(bddGenerator.False)

  txt = Array(
    "Forall user . Forall file . access(user,file) -> [login(user),logout(user)) & [open(file),close(file))",
      "Forall file . access(user,file) -> [login(user),logout(user)) & [open(file),close(file))",
      "access(user,file) -> [login(user),logout(user)) & [open(file),close(file))",
      "access(user,file)",
      "[login(user),logout(user)) & [open(file),close(file))",
